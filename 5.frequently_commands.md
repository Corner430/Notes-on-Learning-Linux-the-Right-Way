...
其中，`options` 是可选参数，可以用于定制生成报告的内容和行为。

常用的 `sosreport` 命令选项和功能如下：
  1. 生成报告：默认情况下，运行 `sosreport` 命令会生成一个包含所有常用信息的完整报告，包括系统信息、内核参数、进程列表、网络配置、日志文件、存储信息、性能数据等。报告会被打包成一个压缩文件，并保存在当前用户的家目录下。
  2. 指定输出目录：使用 `--tmp-dir` 选项，可以指定生成报告的输出目录。例如：
     ```
     sosreport --tmp-dir /path/to/output
     ```
     报告将会保存在指定的目录中。
  3. 压缩级别：使用 `--batch` 选项，可以指定生成报告的压缩级别。例如：
     ```
     sosreport --batch=3
     ```
     压缩级别范围从1到9，1表示最快的压缩速度和最低的压缩率，9表示最慢的压缩速度和最高的压缩率。
  4. 不收集特定信息：使用 `--skip` 选项，可以指定不收集某些信息。例如，不收集日志文件可以使用：
     ```
     sosreport --skip-logs
     ```
  5. 收集特定信息：使用 `--only` 选项，可以只收集某些特定信息。例如，只收集网络配置和接口信息可以使用：
     ```
     sosreport --only-network
     ```
`sosreport` 命令会收集大量的信息，因此在运行时可能需要一些时间来生成报告。生成的报告可以被提交给技术支持团队或系统管理员，用于诊断系统问题、收集性能数据和进行系统调优。
